...
param composizione {Metalli,Materiali}; # Composizione percentuale dei materiali
param pmin {Metalli}; # Percentuale minima richiesta
param pmax {Metalli}; # Percentuale massima ammessa
param disponibilita {Materiali2}; # Quantità di materiali disponibile [ton]
param costi {Materiali}; # Costo dei materiali [€/ton]
param domanda; # Quantità totale da produrre 
# VARIABILI
var x {i in Materiali} >=0;
# VINCOLI
# Vincoli sulla max quantità disponibile 
subject to Max_quantity {i in Materiali2}:
  x[i] <= disponibilita[i];
# Vincoli sul totale peso dei materiali utilizzati
...
subject to Limiti_inf {j in Metalli}:
  sum {i in Materiali} composizione[j,i] * x[i] / domanda >= pmin[j];
subject to Limiti_sup {j in Metalli}:
  sum {i in Materiali} composizione[j,i] * x[i] / domanda <= pmax[j];
# OBIETTIVO
minimize z: sum {i in Materiali} costi[i] * x[i];
#####
data;
...
param composizione :  ALMC    KAC    Rottami   Al    Si    Mg  :=
