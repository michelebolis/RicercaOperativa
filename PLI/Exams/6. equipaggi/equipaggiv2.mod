...
subject to VoloCoperto{v in Voli} :
  sum{s in Sequenze : v in VoliSequenza[s]} x[s] = 1 + voliIdle[v];
subject to nonPeggioramento : sum{s in Sequenze} costi[s] * x[s] <= 8000;
# FUNZIONE OBIETTIVO
minimize costiConMinVoliExtra : sum{s in Sequenze} x[s] * costi[s];
#############
data;
param nVoli := 30;
